<script lang="ts">
  ...
  function logout() { localStorage.removeItem('simplesmente-sim-admin-token'); localStorage.removeItem('simplesmente-sim-admin-user'); token = ''; user = null; overview = null; userSummary = null; users = []; }
</script>
...
{:else}
  <AdminShell userName={user?.name ?? 'Administrador'} onLogout={logout}>
    <div class="page-heading"><div><span class="eyebrow">Workspace administrativo</span><h1>Visão geral</h1><p>Acompanhe a operação e os recursos disponíveis no Simplesmente Sim.</p></div><div class="refresh-control"><button class="refresh" type="button" onclick={refreshData}>Atualizar dados</button>{#key refreshCycle}<span class="refresh-progress" role="progressbar" aria-label="Próxima atualização automática" onanimationend={refreshData}></span>{/key}</div></div>
